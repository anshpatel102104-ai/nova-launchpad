...
async function handleCheckoutCompleted(
  ...
  await fetch(
    `${env.SUPABASE_URL}/rest/v1/organizations?id=eq.${orgId}`,
    {
      method: 'PATCH',
...
async function handleSubscriptionDeleted(
  ...
  await fetch(
    `${env.SUPABASE_URL}/rest/v1/subscriptions?stripe_subscription_id=eq.${stripeSubId}`,
    {
      method: 'PATCH',
      headers: sbHeaders(env),
      body: JSON.stringify({ plan_tier: 'starter', status: 'cancelled', updated_at: new Date().toISOString() }),
    }
  );
...
async function handleCreateCheckout(request: Request, env: Env, origin: string): Promise<Response> {
  ...
  if (!priceId) {
    return jsonResponse({ error: 'Invalid plan. Must be 49, 149, or 299.' }, 400, corsHeaders(origin));
  }
