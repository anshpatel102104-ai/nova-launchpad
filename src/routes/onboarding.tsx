  ...
  const handleComplete = async (answers: OnboardingAnswers) => {
    ...
    if (existingOrg) {
      ...
      const { data: org, error: orgErr } = await supabase
        .from("organizations")
        .insert({ name: "My Workspace", owner_id: user.id, stage: (stage || undefined) as "Idea" | "Validate" | "Launch" | "Operate" | "Scale" | undefined })
        .select("id")
        .single();
    ...
    try {
      const provisionRes = await fetch(`${supabaseBase}/functions/v1/provision-workspace`, {
        ...
        body: JSON.stringify({ organization_id: orgId, name: "My Workspace", lane, stage: stage || "Idea", idea }),
      });
      if (provisionRes.ok) {
...
function WelcomeScreen({ onSkip }: { onSkip: () => void }) {
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "#0a0805", display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
      <style>{ANIM_CSS}</style>

      <div style={{ position: "absolute", inset: 0, opacity: phase === "reveal" ? 0.12 : 0.22, transition: "opacity 1s" }}>
        <NeuralCanvas className="w-full h-full" />
      </div>

      <div style={{
        position: "absolute", width: 800, height: 600, borderRadius: "50%",
        background: "radial-gradient(ellipse, rgba(249,115,22,0.12) 0%, rgba(251,191,36,0.05) 40%, transparent 70%)",
        transition: "opacity 1s", opacity: phase === "reveal" ? 1 : 0.4,
        animation: "ambientPulse 4s ease-in-out infinite", pointerEvents: "none",
      }} />
      ...
      <div style={{ position: "relative", zIndex: 10, textAlign: "center", padding: "0 24px", maxWidth: 600 }}>
        {phase === "boot" && (
          <div style={{ fontFamily: "monospace", textAlign: "left", display: "inline-block" }}>
            {BOOT_LINES.slice(0, lineIdx).map((line, i) => (
              <div key={line} style={{ fontSize: 13, color: "rgba(249,115,22,0.85)", lineHeight: 2.1, animation: "bootLine 0.35s ease both" }}>
        ...
        {phase === "reveal" && (
          <div style={{ animation: "nameReveal 1s cubic-bezier(0.16,1,0.3,1) both" }}>
            <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.2em", textTransform: "uppercase", color: "#f97316", marginBottom: 20, animation: "fadeUp 0.6s ease 0.15s both", opacity: 0 }}>
              System ready
            </div>

            <h1 style={{ fontSize: "clamp(2.6rem, 7vw, 4.5rem)", fontWeight: 900, letterSpacing: "-0.05em", lineHeight: 1.04, color: "#f7f0e8", margin: "0 0 8px", textShadow: "0 0 60px rgba(249,115,22,0.2)" }}>
              Nova is ready.
            </h1>

            <div style={{ margin: "22px auto 0", height: 2, borderRadius: 2, background: "linear-gradient(90deg, transparent, #f97316, #fbbf24, transparent)", boxShadow: "0 0 20px rgba(249,115,22,0.6)", animation: "lineExpand 0.9s cubic-bezier(0.16,1,0.3,1) 0.3s both" }} />

            <p style={{ marginTop: 22, fontSize: 16, color: "rgba(247,240,232,0.45)", lineHeight: 1.65, animation: "fadeUp 0.6s ease 0.5s both", opacity: 0 }}>
              Your AI founder OS is online.<br />Let's build something remarkable.
            </p>

            <div style={{ marginTop: 32, display: "flex", flexDirection: "column", alignItems: "center", gap: 14, animation: "fadeUp 0.6s ease 0.75s both", opacity: 0 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                <div style={{ width: 6, height: 6, borderRadius: "50%", background: "#f97316", boxShadow: "0 0 10px #f97316", animation: "ambientPulse 1.4s ease-in-out infinite" }} />
                <span style={{ fontSize: 12, color: "rgba(247,240,232,0.3)", fontFamily: "monospace" }}>loading dashboard…</span>
              </div>
              <button
                onClick={onSkip}
                style={{ fontSize: 12, color: "rgba(247,240,232,0.5)", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(249,115,22,0.15)", borderRadius: 8, padding: "6px 16px", cursor: "pointer", fontFamily: "inherit", transition: "all 0.2s" }}
              >
              ...
              {showFallback && (
                <div style={{ fontSize: 11, color: "rgba(247,240,232,0.3)", marginTop: 4 }}>
                  Taking too long?{" "}
                  <button onClick={onSkip} style={{ fontSize: 11, color: "#f97316", background: "none", border: "none", cursor: "pointer", fontFamily: "inherit", padding: 0, textDecoration: "underline" }}>
