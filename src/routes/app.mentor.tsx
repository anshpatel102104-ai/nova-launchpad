    ...
    brief: [
      {
        ...
        detail: "LinkedIn is converting at 3× cold email. Focus budget here. Cut channels with CAC > $200.",
        time: "30 min",
    ...
    brief: [
      {
        ...
        detail: "Best customer result → 2-sentence outcome statement. This anchors the entire offer.",
        time: "20 min",
      ...
      },
      {
        ...
        detail: "Replace feature bullets with outcomes: 'Save 12h/week' vs 'Automated workflow engine'.",
        time: "60 min",
    ...
    brief: [
      {
        ...
        detail: "Leads with 3+ proposal opens or same-week discovery calls need contact within 24h or go cold.",
        time: "45 min",
    ...
    brief: [
      {
        ...
        detail: "Founder journey, client transformation stories, tactical how-to posts drive 80% of B2B founder engagement.",
        time: "30 min",
    ...
    brief: [
      {
        ...
        detail: "Score leads on email opens, page visits, response speed. Auto-flag score 70+ for immediate sales contact.",
        time: "40 min",
...
function OrbitalCanvas() {
  ...
  useEffect(() => {
    ...
    const ORBS = [
      ...
      { frac: 0.40, speed: 0.00012, angle: 0.8,      color: C.violet },
      { frac: 0.40, speed: 0.00012, angle: 3.6,      color: C.cyan },
  ...
  return (
    <canvas
      ...
      style={{ position: "absolute", inset: 0, width: "100%", height: "100%", opacity: 0.48, zIndex: 0 }}
    />
  );
}
...
function Sparkline({ data, color, w = 72, h = 22 }: { data: number[]; color: string; w?: number; h?: number }) {
  ...
  const rng = max - min || 1;
  const pts = data.map((v, i) => `${((i / (data.length - 1)) * w).toFixed(1)},${(h - ((v - min) / rng) * (h - 2) - 1).toFixed(1)}`).join(" ");
...
function StatusBadge({ status }: { status: AgentStatus }) {
  ...
  return (
    <span className="inline-flex items-center gap-1 rounded-full px-2 py-0.5"
      style={{ fontSize: "9px", fontWeight: 700, letterSpacing: "0.12em", color: cfg.color, background: `${cfg.color}14`, border: `1px solid ${cfg.color}28` }}>
      <span className={cn("h-1.5 w-1.5 rounded-full", cfg.pulse && "nova-live-dot")} style={{ background: cfg.color }} />
      {cfg.label}
    </span>
  );
}
...
function ConfRing({ value, color, size = 28 }: { value: number; color: string; size?: number }) {
  const r = (size - 4) / 2, c2 = 2 * Math.PI * r;
  return (
    <div className="relative shrink-0" style={{ width: size, height: size }}>
      <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`} style={{ transform: "rotate(-90deg)" }}>
        ...
        <circle cx={size/2} cy={size/2} r={r} fill="none" stroke={color} strokeWidth="2.5" strokeLinecap="round"
          strokeDasharray={c2} strokeDashoffset={c2 - (c2 * value) / 100}
...
function AgentCard({ agent, isSelected, onClick }: { agent: LiveAgent; isSelected: boolean; onClick: () => void }) {
  return (
      ...
      style={{
        ...
        boxShadow: isSelected
          ...
          : agent.status === "active"
            ...
            : undefined,
        background: isSelected ? `color-mix(in oklab, ${agent.color} 5%, var(--surface))` : undefined,
      }}
    >
      {/* Top accent line */}
      <div className="absolute top-0 left-0 right-0 h-px"
        style={{ background: `linear-gradient(90deg, transparent, ${agent.color}${agent.status === "active" ? "70" : "35"}, transparent)` }} />
      {/* Aerospace corner brackets */}
      <div className="absolute top-0 left-0 w-3 h-3 pointer-events-none"
        style={{ borderTop: `1px solid ${agent.color}40`, borderLeft: `1px solid ${agent.color}40` }} />
      <div className="absolute bottom-0 right-0 w-3 h-3 pointer-events-none"
        style={{ borderBottom: `1px solid ${agent.color}28`, borderRight: `1px solid ${agent.color}28` }} />

      <div className="p-4">
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="flex items-center gap-2.5">
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl transition-transform group-hover:scale-105"
            ...
            <div>
              <div className="font-display font-bold leading-tight" style={{ fontSize: "12.5px", color: "var(--foreground)" }}>
                {agent.role}
              </div>
              <div className="font-mono" style={{ fontSize: "9px", color: agent.color, letterSpacing: "0.1em", opacity: 0.8 }}>
        ...
        <div className="text-[11.5px] leading-snug mb-3 line-clamp-2" style={{ color: "rgba(240,244,255,0.5)" }}>
          {agent.objective}
        </div>

        <div className="rounded-lg px-3 py-2 mb-3"
          style={{ background: `${agent.color}08`, border: `1px solid ${agent.color}18` }}>
          <div className="font-mono mb-0.5" style={{ fontSize: "8.5px", color: agent.color, letterSpacing: "0.1em", opacity: 0.7 }}>
            LAST SIGNAL
          </div>
          <div className="text-[11px] leading-snug line-clamp-2" style={{ color: "rgba(240,244,255,0.75)" }}>
            {agent.lastRec}
          </div>
        </div>

        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1.5 text-[10.5px] truncate" style={{ color: "rgba(240,244,255,0.4)" }}>
            <ArrowRight className="h-3 w-3 shrink-0" style={{ color: agent.color }} />
            <span className="truncate">{agent.nextMove}</span>
          </div>
          <ChevronRight className="h-3.5 w-3.5 shrink-0 ml-2 transition-transform group-hover:translate-x-0.5"
...
function MissionBriefPanel({
  agent, orgId, userId, accessToken, onClose,
}: {
  agent: LiveAgent;
  orgId: string;
  userId: string;
  ...
  const sendChat = async () => {
    ...
    const userMsg: MentorMessage = { role: "user", text: input.trim(), ts: new Date().toISOString() };
    setSending(true);
    setInput("");
    ...
    try {
      ...
      const agentMsg: MentorMessage = {
        role: "agent",
        text: result.success && result.response ? result.response : (result.error ?? "Unable to respond right now. Try again."),
  ...
  return (
    <>
      <div className="fixed inset-0 z-40" style={{ background: "rgba(8,8,16,0.65)", backdropFilter: "blur(3px)" }} onClick={onClose} />
      <div className="fixed right-0 top-0 bottom-0 z-50 flex flex-col overflow-hidden slide-in-right"
        style={{
        ...
        {/* Header */}
        <div className="flex items-center gap-3 px-5 py-4 shrink-0" style={{ borderBottom: `1px solid ${agent.color}18` }}>
          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl"
            style={{ background: `${agent.color}14`, border: `1px solid ${agent.color}28`, boxShadow: `0 0 14px ${agent.color}22` }}>
            <agent.Icon className="h-4 w-4" style={{ color: agent.color }} />
          </div>
          <div className="flex-1 min-w-0">
            <div className="font-display font-bold" style={{ fontSize: "13.5px", color: "var(--foreground)" }}>{agent.role}</div>
            <div className="flex items-center gap-2">
              <StatusBadge status={agent.status} />
              <span className="font-mono" style={{ fontSize: "9px", color: agent.color, letterSpacing: "0.08em", opacity: 0.7 }}>
          ...
          <button onClick={onClose} className="flex h-7 w-7 items-center justify-center rounded-lg transition-colors"
            ...
            onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.color = "rgba(240,244,255,0.3)"; }}>
            <X className="h-4 w-4" />
          </button>
        </div>
        <div className="h-px shrink-0" style={{ background: `linear-gradient(90deg, ${agent.color}, ${agent.color}00)`, opacity: 0.6 }} />

        <div className="flex-1 overflow-y-auto">
          {/* Active mission */}
          <div className="px-5 py-4" style={{ borderBottom: `1px solid ${agent.color}10` }}>
            <div className="font-mono mb-1" style={{ fontSize: "8.5px", color: agent.color, letterSpacing: "0.14em", opacity: 0.7 }}>
              ACTIVE MISSION
            </div>
            <div className="font-display font-bold text-[14px]" style={{ color: "var(--foreground)" }}>
              {agent.objective}
            </div>
          </div>
          ...
          <div className="px-5 py-4" style={{ borderBottom: `1px solid ${agent.color}10` }}>
            <div className="flex items-center justify-between mb-3">
              <span className="font-mono" style={{ fontSize: "8.5px", color: agent.color, letterSpacing: "0.14em", opacity: 0.7 }}>
                EXECUTION SEQUENCE
              </span>
              ...
              {agent.brief.map((step, i) => (
                <div key={i} onClick={() => toggleDone(i)}
                  ...
                  style={{
                    ...
                    border: done.has(i) ? "1px solid rgba(16,185,129,0.2)" : `1px solid ${agent.color}18`,
                  ...
                  <div className="flex items-start gap-3 p-3">
                    <div className="mt-0.5 shrink-0">
                      {done.has(i)
                        ...
                        : <Circle className="h-4 w-4" style={{ color: `${priorityColor[step.priority]}80` }} />}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap mb-1">
                        <span className={cn("font-semibold text-[12.5px]", done.has(i) && "line-through opacity-50")}
                          style={{ color: "var(--foreground)" }}>
                          {step.title}
                        </span>
                        <span className="rounded-full px-1.5 py-0.5 font-mono"
                          style={{ fontSize: "8px", fontWeight: 700, letterSpacing: "0.1em", color: priorityColor[step.priority], background: `${priorityColor[step.priority]}14` }}>
                          {step.priority.toUpperCase()}
                        </span>
                        <span className="flex items-center gap-1 text-[10px]" style={{ color: "rgba(240,244,255,0.3)" }}>
                          <Clock className="h-2.5 w-2.5" />{step.time}
                        </span>
                      </div>
                      <p className="text-[11.5px] leading-relaxed mb-2" style={{ color: "rgba(240,244,255,0.5)" }}>
                        {step.detail}
                      </p>
                      <div className="flex items-center gap-1.5">
                        <div className="flex items-center gap-1.5 text-[10.5px] font-medium" style={{ color: agent.color }}>
                          <ArrowRight className="h-3 w-3" />{step.action}
                        </div>
                        {step.toolLink && (
                          <Link to={step.toolLink} onClick={(e) => e.stopPropagation()}
          ...
          {/* Chat history */}
          {messages.length > 0 && (
            <div className="px-5 py-4 space-y-3" style={{ borderBottom: `1px solid ${agent.color}10` }}>
              <div className="font-mono" style={{ fontSize: "8.5px", color: agent.color, letterSpacing: "0.12em", opacity: 0.6 }}>
                CONVERSATION LOG
              </div>
              {messages.map((m, i) => (
                <div key={i} className={cn("flex", m.role === "user" ? "justify-end" : "justify-start")}>
                  <div className="max-w-[85%] rounded-xl px-3 py-2 text-[12px] leading-relaxed"
                    style={m.role === "user"
                      ? { background: `${agent.color}18`, border: `1px solid ${agent.color}28`, color: "var(--foreground)" }
                      : { background: "var(--surface-2)", border: "1px solid rgba(255,255,255,0.06)", color: "rgba(240,244,255,0.8)" }}>
                    {m.text}
                  </div>
                </div>
              ))}
        ...
        <div className="shrink-0 p-4" style={{ borderTop: `1px solid ${agent.color}18` }}>
          <div className="font-mono mb-2" style={{ fontSize: "8.5px", color: agent.color, letterSpacing: "0.12em", opacity: 0.6 }}>
            ASK {agent.codename}
          </div>
          <div className="flex gap-2">
            <input type="text" value={input} onChange={(e) => setInput(e.target.value)}
              ...
              disabled={sending} />
            <button onClick={sendChat} disabled={!input.trim() || sending}
              className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg transition-all duration-150 disabled:opacity-30"
              style={{ background: `${agent.color}18`, border: `1px solid ${agent.color}28`, color: agent.color }}
              onMouseEnter={(e) => { if (input.trim()) { (e.currentTarget as HTMLElement).style.boxShadow = `0 0 12px ${agent.color}30`; } }}
...
function CommandCenter({
  agents, userId, orgId, accessToken, onClose,
}: {
  agents: LiveAgent[];
  userId: string;
  orgId: string;
  ...
  const [responses, setResponses] = useState<Array<{ agent: LiveAgent; text: string; loading: boolean }>>([]);
      ...
      activeAgents.map(async (a, i) => {
        try {
          const res = await runMentorAgent(
            userId,
            { agent_id: a.id, message: query, org_id: orgId, business_context: `Role: ${a.role}. Objective: ${a.objective}.` },
            accessToken,
          );
          setResponses((prev) => {
            const next = [...prev];
            next[i] = { agent: a, text: res.success && res.response ? res.response : (res.error ?? "No response"), loading: false };
            return next;
          });
        } catch {
          setResponses((prev) => { const next = [...prev]; next[i] = { agent: a, text: "Agent unreachable.", loading: false }; return next; });
        }
      }),
    );
  };
  ...
  return (
    <>
      <div className="fixed inset-0 z-40" style={{ background: "rgba(8,8,16,0.85)", backdropFilter: "blur(8px)" }} onClick={onClose} />
      <div className="animate-scale-in fixed inset-x-4 inset-y-6 z-50 mx-auto flex flex-col overflow-hidden rounded-2xl sm:inset-x-8 sm:inset-y-8"
        style={{
          maxWidth: "840px",
          background: "var(--surface)",
        ...
        {/* Header */}
        <div className="flex items-center gap-3 px-6 py-4 shrink-0" style={{ borderBottom: "1px solid rgba(59,130,246,0.12)" }}>
          ...
          <div>
            <div className="font-display font-bold text-[14px]" style={{ color: "var(--foreground)" }}>Command Center</div>
          ...
          <button onClick={onClose} className="ml-auto flex h-7 w-7 items-center justify-center rounded-lg" style={{ color: "rgba(240,244,255,0.3)" }}>
            <X className="h-4 w-4" />
          </button>
        </div>

        {/* Agent pills */}
        <div className="flex gap-2 px-6 py-3 overflow-x-auto shrink-0" style={{ borderBottom: "1px solid rgba(59,130,246,0.08)" }}>
          {activeAgents.map((a) => (
            <div key={a.id} className="flex items-center gap-2 rounded-lg px-3 py-1.5 shrink-0"
              style={{ background: `${a.color}10`, border: `1px solid ${a.color}25` }}>
              <a.Icon className="h-3 w-3" style={{ color: a.color }} />
              <span className="text-[11px] font-medium" style={{ color: "rgba(240,244,255,0.8)" }}>{a.role}</span>
              <span className="h-1.5 w-1.5 rounded-full nova-live-dot" style={{ background: a.color }} />
            </div>
          ))}
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          {!sent ? (
            <div>
              <div className="font-mono mb-4" style={{ fontSize: "9px", color: "rgba(59,130,246,0.7)", letterSpacing: "0.14em" }}>
              ...
              <div className="space-y-2.5">
                {STARTERS.map((s) => (
                  <button key={s} onClick={() => setQuery(s)}
                    ...
                    style={{ background: "var(--surface-2)", border: "1px solid rgba(59,130,246,0.1)", color: "rgba(240,244,255,0.65)" }}
                    onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.borderColor = "rgba(59,130,246,0.3)"; (e.currentTarget as HTMLElement).style.color = "var(--foreground)"; }}
                    onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.borderColor = "rgba(59,130,246,0.1)"; (e.currentTarget as HTMLElement).style.color = "rgba(240,244,255,0.65)"; }}>
                    {s}
                  </button>
                ))}
              </div>
            ...
            <div className="space-y-3">
              <div className="rounded-xl px-4 py-3 text-[13px]"
                style={{ background: "rgba(59,130,246,0.08)", border: "1px solid rgba(59,130,246,0.2)", color: "var(--foreground)" }}>
                {query}
              </div>
              {responses.map((r, i) => (
                <div key={r.agent.id} className="rounded-xl p-4"
                  style={{ background: `${r.agent.color}06`, border: `1px solid ${r.agent.color}18`,
                    animation: `riseIn 0.4s ease both`, animationDelay: `${i * 80}ms` }}>
                  <div className="flex items-center gap-2 mb-2">
                    <r.agent.Icon className="h-3.5 w-3.5" style={{ color: r.agent.color }} />
                    <span className="font-mono" style={{ fontSize: "9px", fontWeight: 700, color: r.agent.color, letterSpacing: "0.1em" }}>
                      {r.agent.codename} · {r.agent.role.toUpperCase()}
                    </span>
                    {r.loading && <Loader2 className="h-3 w-3 animate-spin ml-auto" style={{ color: r.agent.color }} />}
                  </div>
                  {r.loading
                    ? <div className="skeleton h-12 w-full rounded-lg" />
                    : <p className="text-[12.5px] leading-relaxed" style={{ color: "rgba(240,244,255,0.75)" }}>{r.text}</p>}
                </div>
              ))}
            </div>
          )}
        ...
        <div className="shrink-0 p-4" style={{ borderTop: "1px solid rgba(59,130,246,0.1)" }}>
          <div className="flex gap-2">
            <input type="text" value={query} onChange={(e) => setQuery(e.target.value)}
              ...
              className="terminal-input flex-1 px-3 py-2.5 text-[12.5px]" autoFocus />
            <button onClick={handleSubmit} disabled={!query.trim() || sent}
...
function MentorPage() {
  ...
  const insightsQ = useQuery({ ...mentorInsightsQuery(currentOrgId ?? ""), enabled: !!currentOrgId });
  ...
  const pipelineStages = [
    ...
    { label: "Qualified",  count: leads.filter((l) => ["Qualified","Proposal"].includes(l.stage as string)).length, color: C.cyan,   active: false },
    { label: "Proposal",   count: leads.filter((l) => l.stage === "Proposal").length,         color: C.violet, active: true  },
    { label: "Negotiation",count: leads.filter((l) => l.stage === "Negotiation").length,     color: C.amber,  active: false },
    { label: "Won",        count: leads.filter((l) => l.stage === "Won").length,              color: C.green,  active: false },
  ];
  ...
  const liveAgents: LiveAgent[] = AGENT_DEFS.map((def) => {
    ...
    switch (def.id) {
      case "growth":
        ...
        confidence = Math.min(95, 60 + Math.min(runs.filter(r => r.status === "succeeded").length * 2, 30));
        lastRec = leads.length > 5
          ? `${leads.length} leads in pipeline — optimize top-of-funnel for MRR growth`
          : "Start capturing leads — pipeline is empty";
        nextMove = runs.length === 0 ? "Run Idea Validator or GTM Strategy" : "Scale best-performing channel";
        objective = `Scale MRR from $${(kpis.mrr).toLocaleString()} → $10k in 60 days`;
        break;
      case "offer":
        status = runs.some(r => r.tool_key === "generate-offer" && r.status === "succeeded") ? "active" : "analyzing";
        ...
        break;
      case "sales":
        status = leads.some(l => ["Qualified","Proposal"].includes(l.stage as string)) ? "active" : "standby";
        confidence = Math.min(95, 60 + kpis.wonLeads * 5 + pipelineStages[1].count * 3);
        lastRec = pipelineStages[2].count > 0
          ? `${pipelineStages[2].count} proposals out — follow up within 24h to avoid going cold`
          : leads.length > 0 ? "Qualify your leads and move them to Proposal stage" : "Capture first leads to activate sales pipeline";
        nextMove = pipelineStages[2].count > 0 ? "Send follow-ups to proposal-stage leads" : "Add leads to CRM";
        objective = `Close ${Math.max(1, pipelineStages[1].count)} deals from ${leads.length}-lead pipeline`;
        break;
      case "content":
        status = runs.some(r => r.tool_key === "blog" && r.status === "succeeded") ? "active" : "standby";
        confidence = 64;
      ...
      case "automation":
        ...
        lastRec = kpis.activeAutomations > 0
  ...
  const derivedInsights = [
    pipelineStages[2].count > 0 && {
      id: "d1", agent: "Sales Operator", color: C.green, type: "signal" as const,
      ...
      ago: "live", priority: "high" as const, read: false,
      org_id: currentOrgId ?? "", agent_id: "sales", n8n_run_id: null, created_at: "",
    },
    (kpis?.cacRatio ?? 0) < 3 && kpis?.cacRatio !== 0 && {
      id: "d2", agent: "Finance Navigator", color: C.amber, type: "warning" as const,
      ...
      ago: "live", priority: "medium" as const, read: false,
      org_id: currentOrgId ?? "", agent_id: "finance", n8n_run_id: null, created_at: "",
    },
    kpis && kpis.activeAutomations === 0 && {
      id: "d3", agent: "Automation Engineer", color: C.cyan, type: "recommendation" as const,
      ...
      ago: "live", priority: "medium" as const, read: false,
      org_id: currentOrgId ?? "", agent_id: "automation", n8n_run_id: null, created_at: "",
    },
  ...
  if (!currentOrgId) {
    return (
      <div className="flex min-h-[60vh] items-center justify-center text-center">
        <div>
          <div className="font-mono text-[10px] mb-2" style={{ color: "rgba(59,130,246,0.6)", letterSpacing: "0.2em" }}>
            MISSION CONTROL
          </div>
  ...
  return (
    <div className="relative min-h-full space-y-5 pb-24">
      ...
      {/* ── MISSION STATUS HERO ──────────────────────────────────── */}
      <section className="rise-in relative overflow-hidden rounded-2xl"
        style={{
          ["--i" as string]: 0, minHeight: 180,
          background: "var(--surface)",
          border: "1px solid rgba(59,130,246,0.14)",
          boxShadow: "0 0 0 1px rgba(59,130,246,0.06), 0 1px 3px rgba(0,0,0,0.6), 0 8px 40px rgba(0,0,0,0.45)",
        }}>
        <OrbitalCanvas />
        <div className="absolute inset-0 z-[1]"
          style={{ background: "linear-gradient(135deg, rgba(8,8,16,0.88) 0%, rgba(13,13,30,0.72) 55%, rgba(8,8,16,0.82) 100%)" }} />
        <div className="absolute top-0 left-0 right-0 h-px z-[2]"
          style={{ background: "linear-gradient(90deg, transparent, rgba(59,130,246,0.5), rgba(139,92,246,0.35), transparent)" }} />
        ...
        <div className="relative z-[3] flex flex-col gap-4 p-5 md:flex-row md:items-center md:p-7">
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-3 mb-3">
              <span className="font-mono" style={{ fontSize: "9.5px", fontWeight: 700, letterSpacing: "0.2em", color: "rgba(59,130,246,0.8)" }}>
            ...
            <h1 className="font-display font-black leading-none mb-2"
              style={{
                fontSize: "clamp(1.5rem, 2.5vw + 0.5rem, 2.2rem)", letterSpacing: "-0.04em",
                background: "linear-gradient(125deg, #ffffff 0%, rgba(255,255,255,0.82) 45%, #60a5fa 100%)",
                WebkitBackgroundClip: "text", backgroundClip: "text", color: "transparent",
              }}>
              Business Mission Control
            </h1>
            <p style={{ fontSize: "13px", color: "rgba(240,244,255,0.5)", lineHeight: 1.6 }}>
              {liveAgents.length} AI mentors monitoring your business. {allInsights.length} signals loaded.
            </p>
          </div>
          ...
          <div className="flex gap-3 md:flex-col md:items-end md:gap-3">
            <div className="flex gap-3 flex-wrap">
              {[
                ...
                <div key={label} className="rounded-xl px-3 py-2 text-center"
                  ...
                  <div className="font-mono" style={{ fontSize: "8px", color: `${color}aa`, letterSpacing: "0.1em" }}>{label}</div>
                  <div className="font-mono font-black tabular-nums telem-flicker mt-0.5"
                    style={{ fontSize: "16px", color, letterSpacing: "-0.02em" }}>{value}</div>
                </div>
              ))}
            </div>
            <button onClick={() => setShowCC(true)}
              ...
              style={{ background: "linear-gradient(135deg, #3b82f6, #6366f1)", boxShadow: "0 4px 16px rgba(59,130,246,0.35), inset 0 1px 0 rgba(255,255,255,0.15)" }}
              onMouseEnter={(e) => { (e.currentTarget as HTMLElement).style.transform = "translateY(-2px)"; (e.currentTarget as HTMLElement).style.boxShadow = "0 8px 24px rgba(59,130,246,0.5), inset 0 1px 0 rgba(255,255,255,0.15)"; }}
              onMouseLeave={(e) => { (e.currentTarget as HTMLElement).style.transform = "none"; (e.currentTarget as HTMLElement).style.boxShadow = "0 4px 16px rgba(59,130,246,0.35), inset 0 1px 0 rgba(255,255,255,0.15)"; }}>
      ...
      {/* ── KPI ROW ─────────────────────────────────────────────── */}
      <section className="rise-in grid gap-3 sm:grid-cols-2 lg:grid-cols-4" style={{ ["--i" as string]: 1 }}>
        {KPI_CARDS.map((kpi) => (
          <div key={kpi.label} className="nova-card nova-card-hover relative overflow-hidden">
            ...
            <div className="p-4">
              <div className="font-mono mb-2" style={{ fontSize: "8.5px", fontWeight: 700, letterSpacing: "0.14em", color: "rgba(240,244,255,0.4)" }}>
                {kpi.label.toUpperCase()}
              </div>
              <div className="flex items-end justify-between mb-1">
                <div className="font-display font-black tabular-nums leading-none"
                  style={{ fontSize: "1.5rem", color: "var(--foreground)", letterSpacing: "-0.04em" }}>
                  {kpiQ.isLoading ? <span className="skeleton inline-block w-20 h-7 rounded" /> : kpi.value}
                </div>
                <Sparkline data={kpi.sparkline} color={kpi.color} />
              </div>
              <div className="flex items-center gap-2">
                <span className="rounded-full px-1.5 py-0.5 font-mono text-[9px] font-bold"
                  style={{ background: kpi.positive ? "rgba(16,185,129,0.12)" : "rgba(248,113,113,0.12)", color: kpi.positive ? C.green : C.red }}>
                  {kpi.change}
                </span>
                <span style={{ fontSize: "10.5px", color: "rgba(240,244,255,0.35)" }}>{kpi.sub}</span>
              </div>
            </div>
          </div>
        ))}
      ...
      <section className="rise-in" style={{ ["--i" as string]: 2 }}>
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-3">
            <div className="font-mono" style={{ fontSize: "9px", fontWeight: 700, letterSpacing: "0.18em", color: "rgba(59,130,246,0.7)" }}>
              AGENT FLEET
            </div>
          ...
          <div className="flex items-center gap-2">
            {(["active", "analyzing", "standby"] as const).map((s) => {
              ...
              const clr = s === "active" ? C.green : s === "analyzing" ? C.blue : "rgba(240,244,255,0.2)";
              return (
                <span key={s} className="font-mono rounded-full px-2 py-0.5"
                  style={{ fontSize: "9px", fontWeight: 700, letterSpacing: "0.08em", color: clr, background: `${clr}12`, border: `1px solid ${clr}25` }}>
                  {cnt} {s}
                </span>
              );
            })}
      ...
      <section className="rise-in grid gap-4 lg:grid-cols-12" style={{ ["--i" as string]: 3 }}>
        ...
        <div className="lg:col-span-7 overflow-hidden rounded-2xl"
          ...
          <div className="flex items-center justify-between px-5 py-4" style={{ borderBottom: "1px solid rgba(59,130,246,0.08)" }}>
            <div>
              <div className="font-mono mb-0.5" style={{ fontSize: "8.5px", fontWeight: 700, letterSpacing: "0.16em", color: "rgba(59,130,246,0.6)" }}>
                EXECUTION PIPELINE
              </div>
              <div className="font-display font-bold text-[13.5px]" style={{ color: "var(--foreground)" }}>Sales Trajectory</div>
            </div>
            <Link to="/app/nova/crm" className="inline-flex items-center gap-1 text-[11px] transition-colors" style={{ color: "rgba(59,130,246,0.7)" }}>
          ...
          <div className="p-5">
            <div className="flex items-stretch gap-1.5 mb-4">
              {pipelineStages.map((stage, i) => {
                const width = Math.max(40, stage.count > 0 ? Math.min(200, 40 + stage.count * 8) : 40);
                return (
                  <div key={stage.label} className="flex flex-col" style={{ minWidth: width }}>
                    <div className="rounded-lg relative overflow-hidden flex-1"
                      style={{
                        minHeight: 48, padding: "8px",
                      ...
                      {stage.active && <div className="absolute top-0 left-0 right-0 h-px" style={{ background: stage.color, opacity: 0.6 }} />}
                      <div className="font-mono font-black tabular-nums" style={{ fontSize: "14px", color: stage.color, letterSpacing: "-0.02em" }}>
                        {stage.count}
                      </div>
                    </div>
                    <div className="mt-1 font-mono text-center"
                      style={{ fontSize: "7.5px", color: stage.active ? stage.color : "rgba(240,244,255,0.3)", letterSpacing: "0.04em", lineHeight: 1.3 }}>
                      {stage.label}
                    </div>
                  </div>
                );
            ...
            {pipelineStages[2].count > 0 ? (
              <div className="rounded-xl px-4 py-3 flex items-center gap-3"
                ...
                <Link to="/app/nova/crm">
                  <button className="shrink-0 rounded-lg px-3 py-1.5 text-[11px] font-semibold"
                    style={{ background: "rgba(139,92,246,0.12)", border: "1px solid rgba(139,92,246,0.25)", color: C.violet }}>
                    Act Now
                  </button>
                </Link>
              </div>
            ) : (
              <div className="rounded-xl px-4 py-3 text-center" style={{ background: "rgba(59,130,246,0.04)", border: "1px solid rgba(59,130,246,0.1)" }}>
                ...
                <Link to="/app/nova/leads">
                  <button className="mt-2 rounded-lg px-3 py-1.5 text-[11px] font-semibold"
                    style={{ background: "rgba(59,130,246,0.1)", border: "1px solid rgba(59,130,246,0.2)", color: C.blue }}>
                    Add Leads
                  </button>
                </Link>
              </div>
        ...
        {/* Signal feed */}
        <div className="lg:col-span-5 overflow-hidden rounded-2xl flex flex-col"
          style={{ background: "var(--surface)", border: "1px solid rgba(59,130,246,0.1)" }}>
          <div className="flex items-center justify-between px-5 py-4 shrink-0" style={{ borderBottom: "1px solid rgba(59,130,246,0.08)" }}>
            <div>
              <div className="font-mono mb-0.5" style={{ fontSize: "8.5px", fontWeight: 700, letterSpacing: "0.16em", color: "rgba(59,130,246,0.6)" }}>
                SIGNAL FEED
              </div>
              <div className="font-display font-bold text-[13.5px]" style={{ color: "var(--foreground)" }}>Strategic Insights</div>
            </div>
            <div className="flex items-center gap-1">
              {(["all", "high", "warning"] as const).map((f) => (
                <button key={f} onClick={() => setInsightFilter(f)}
                  ...
                  style={insightFilter === f
                    ? { background: "rgba(59,130,246,0.15)", color: C.blue, border: "1px solid rgba(59,130,246,0.3)" }
          ...
          <ul className="divide-y flex-1 overflow-y-auto" style={{ borderColor: "rgba(59,130,246,0.06)" }}>
            {filteredInsights.length === 0 ? (
              ...
              const IconMap = { signal: Signal, opportunity: ArrowUpRight, warning: AlertTriangle, recommendation: Zap };
              const TypeIcon = IconMap[ins.type];
              const typeColor = { signal: C.blue, opportunity: C.green, warning: C.amber, recommendation: C.cyan }[ins.type];
              return (
                <li key={ins.id} className="flex gap-3 px-5 py-3 transition-all duration-150"
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-start justify-between gap-2 mb-0.5">
                      <div className="font-medium text-[12px] leading-snug" style={{ color: "var(--foreground)" }}>{ins.title}</div>
                      {ins.priority === "high" && (
                        <span className="shrink-0 rounded-full px-1.5 py-0.5 font-mono text-[8px] font-bold" style={{ color: C.red, background: "rgba(248,113,113,0.1)" }}>HIGH</span>
                      )}
                    </div>
                    <div className="text-[11px]" style={{ color: "rgba(240,244,255,0.4)" }}>{ins.detail}</div>
                    <div className="flex items-center gap-1.5 mt-1">
                      <span className="h-1.5 w-1.5 rounded-full shrink-0" style={{ background: ins.color }} />
                      <span className="text-[10px]" style={{ color: ins.color, opacity: 0.8 }}>{ins.agent}</span>
                      {ins.ago && <span className="text-[10px]" style={{ color: "rgba(240,244,255,0.2)" }}>· {ins.ago}</span>}
                    </div>
                  </div>
                </li>
              );
      ...
      <section className="rise-in grid gap-4 lg:grid-cols-12" style={{ ["--i" as string]: 4 }}>
        <div className="lg:col-span-8 relative overflow-hidden rounded-2xl"
          style={{ background: "var(--surface)", border: "1px solid rgba(139,92,246,0.15)", boxShadow: "0 0 30px rgba(139,92,246,0.05)" }}>
          <div className="absolute top-0 left-0 right-0 h-px"
            style={{ background: "linear-gradient(90deg, transparent, rgba(139,92,246,0.5), transparent)" }} />
          <div className="p-5">
            <div className="flex items-center justify-between mb-4">
              <div>
                <div className="font-mono mb-1" style={{ fontSize: "8.5px", fontWeight: 700, letterSpacing: "0.16em", color: "rgba(139,92,246,0.7)" }}>
                  WEEKLY MISSION
                </div>
                <div className="font-display font-bold text-[14px]" style={{ color: "var(--foreground)" }}>
            ...
            <div className="space-y-2">
              {liveAgents.filter((a) => a.status !== "standby").map((agent, i) => (
                <div key={agent.id} className="flex items-center gap-3 rounded-xl px-4 py-2.5"
                  ...
                  <span className="flex-1 text-[12.5px]" style={{ color: "var(--foreground)" }}>{agent.nextMove}</span>
        ...
        <div className="lg:col-span-4 space-y-3">
          <div className="font-mono" style={{ fontSize: "8.5px", fontWeight: 700, letterSpacing: "0.16em", color: "rgba(59,130,246,0.5)" }}>
            COMMAND QUEUE
          </div>
          {[
            ...
            { label: "Run revenue projector",      to: "/app/launchpad/revenue-projector", color: C.amber },
            ...
            <Link key={cmd.to} to={cmd.to}>
              <div className="nova-card nova-card-hover flex items-center gap-3 px-4 py-3 cursor-pointer transition-all">
                <span className="h-1.5 w-1.5 rounded-full nova-live-dot shrink-0" style={{ background: cmd.color }} />
                <span className="flex-1 text-[12px] font-medium" style={{ color: "rgba(240,244,255,0.75)" }}>{cmd.label}</span>
                <ArrowRight className="h-3.5 w-3.5 shrink-0" style={{ color: cmd.color, opacity: 0.6 }} />
              </div>
            </Link>
          ))}
        </div>
