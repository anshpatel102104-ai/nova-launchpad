...
| 8 — Design system governance | ✅ shipped | Domain accent tokens (`--domain-build/customers/automate/insights/nova`) wired through Tailwind theme; **SectionTabs** primitive unifies Customers/Path/Insights into tabbed sections (mounted on 9 pages); **ModuleBoundary** per-module error isolation on Home + Insights; mobile tab bar relabeled to the 7-destination IA; non-blocking query prefetch in /app beforeLoad (§10); ESLint token-governance rule (raw hex in inline styles → warning register, 194 sites) |
